...
class Piece 
  ...
  def initialize (point_array, board)
    ...
    @base_position = [5, 0] # [column, row] probably we measure canvas in this units, so this is in the middle of 0 row
    @board = board
    @moved = true
  end
  ...
  def move (delta_x, delta_y, delta_rotation)
    ...
    # for a piece we have an array of possible rotations (min1- max4) say [r1,r2, r3,r4]
    # when we create a Piece we choose random rotation say r2 
    # now we have delta_rotation say 2 - new rotation will be r4 (if possible)
    # so in rotation methods we use move only with 3d argument
    ...
    moved # just returns @move, so move returns true or false 
  end
...
class Board
  ...
  def initialize (game)
    ...
    @current_block = Piece.next_piece(self) # so we use class method next_piece (random choice of a piece with given board)
    @score = 0
    @game = game
    @delay = 500
  end
  ...
  # gets the next piece
  def next_piece
    @current_block = Piece.next_piece(self) # again we use class method to redefine @current block
    @current_pos = nil
  end
...
class Tetris
  ...
  # buttons created using TetrisButton class from graphics
  def buttons
        
    # 3 buttons for pause, new game and quit   
    ...
    # 5 buttons for movement: left, right, 2 rotate and drop
