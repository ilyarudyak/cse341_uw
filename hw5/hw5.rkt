...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(mlet? e) 
         ...
         ;; and the function’s argument to the result of the second subexpression.
        [(call? e)
         (let* ([clos (eval-under-env (call-funexp e) env)]
           ...
           (if (not (closure? clos))
               (error "you can call only function")
               (if (not (equal? fname #f))
                   ;; if the function name field is NOT #f
                   (eval-under-env fbody ;; closure’s function’s body
                                   (cons
                                   ;; map the function’s name to the closure
                                   (cons fname clos)
                                   ;; map the function argument to the actual value
                                   (cons (cons farg arg) cenv)))
                   ;; if the function name field IS #f
                   (eval-under-env fbody ;; closure’s function’s body
                                   ;; map the function argument to the actual value
                                   (cons (cons farg arg) cenv)))))]     
...
;; return mupl expression; if e1 == aunit then e2 else e3
